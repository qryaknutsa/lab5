...
//        Parser.fillcollection(FileReader.readFile("/home/s336385/lab5/json.json"));
    ...
    static void method(Scanner in, Command command) {
        while (true) {
            System.out.print("~ ");
            try {
                String commandName = in.nextLine();
                Receiver.history.add(commandName.split(" ")[0]);
                if (commandName.trim().length() != 0)
                    command.executeCommand(commandName);
                System.out.println("Введите команду:");
            } catch (NoSuchElementException e) {
                System.out.println("Я не смогла возобновить ввод с консоли, поэтому ошибка просто ловится и выходит из программы. :(");
                break;
            } catch (Exception e) {
                e.printStackTrace();
            }
